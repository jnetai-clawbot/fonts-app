...
object FontManager {
    ...
    val allFonts: List<FontStyle>
        get() {
            ...
            val showTtf = SettingsManager.isShowTtfFonts()
            val visible = (fonts + customFonts).filter { font ->
                if (hidden.contains(font.name)) return@filter false
                if (!font.isUnicode && !showTtf) return@filter false
                true
            }
